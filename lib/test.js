...
let   field = "/dhwCircuits/dhw1/switchPrograms/A",
    ...
    function Daten(body) {
        ...
        var aeskey = new Buffer(passkey, 'hex');
        var Ecb = new MCrypt('rijndael-128', 'ecb');
        Ecb.open(aeskey);

        var s = Ecb.decrypt(new Buffer(b, 'hex')).toString('utf8');
        while (s.charCodeAt(s.length - 1) === 0)
            s = s.slice(0, s.length - 1);

        var info = JSON.parse(s);

        A.If('Daten von %s%s was %O:',server,field,info);
...
//    get(field);
    get('/recordings?timeframe=today');
    get('/recordings/dhwCircuits/dhw1/actualTemp?timeframe=today');
//    bl.close();
}