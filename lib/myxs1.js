...
//                    logj("I should end up here with a response",obj);
                    callb(null,obj);
                } else {
    ...
    that.setState = function(name,value,callback) {
        ...
        if (!fn)
            return logs("MyXS1.setState Name not found: "+name);
...
//        logs(name+", "+ fn + ", " + id + ", " + styp +", " + value + ", "+ val);
    ...
    that.getHistory = function(name,callback,from_s,to_s) {
        if (!name && ! callback)
            return logs("MyXS1.getHistory argument error:("+name+","+callback+","+from_s+","+to_s);
        var nn = that.getName(name);
        if (!nn)
            return logs("MyXS1.getHistory id not found:("+name+","+callback+","+from_s+","+to_s);
    ...
    that.getStatistics = function(name,callback,from_s,to_s) {
        if (!name && ! callback)
            return logs("MyXS1.getHistory argumen error:("+name+","+callback+","+from_s+","+to_s);
        from_s = Math.floor((from_s || Date.now()-1000*60*60*24*365)/1000);
        to_s = Math.floor((to_s ||Date.now())/1000);
        var id = that.getNumber(name);
        var styp = that.getStyp(name);
        logs("get_state_"+styp+"&number="+id + "&sutime="+from_s+"&eutime="+to_s+"&statistics");
        that.sendXS1("get_state_"+styp+"&number="+id + "&sutime="+from_s+"&eutime="+to_s+"&statistics", function(err,obj) {
//            logj("stat got",obj);
            if (err)
                return callback([]);
            callback(obj.statistics);
        });
